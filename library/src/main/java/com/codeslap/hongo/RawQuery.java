...
/**
 * Methods in this interface will work like the findAll* methods in the {@link SqlAdapter}
 * interface, but instead of returning a {@link java.util.List} of beans it will return a cursor.
 *
 * @author cristian
 */
public interface RawQuery {
  ...
  /**
   * Retrieves all elements from the database that are children of the specified parent
   *
   * @param where  the sample object
   * @param parent the parent of the returned items
   * @return a Cursor pointing to the filter rows
   */
  Cursor findAll(Object where, Object parent);
   ...
   *                      HAVING itself). Passing null will cause all row groups to be included, and
   *                      is required when row grouping is not being used.
   ...
   *         synchronized, see the documentation for more details.
   */
  Cursor findAll(String table, String[] projection, String selection, String[] selectionArgs,
