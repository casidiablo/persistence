...
public class HongoConfig {

  private static final Map<String, DatabaseSpec> DB_SPECS = new HashMap<String, DatabaseSpec>();
  private static final Map<String, HongoPrefs> PREFS = new HashMap<String, HongoPrefs>();

  /**
   * This is the spec id used by default when registering using this method {@link
   * HongoConfig#registerSpec(int)}
   */
  public static final String DEFAULT_SPEC_ID = "com.codeslap.hongo.default_spec";
  ...
  public static HongoPrefs getPreference(String name) {
    if (PREFS.containsKey(name)) {
      return PREFS.get(name);
    }
    HongoPrefs hongoPrefs = new HongoPrefs();
    PREFS.put(name, hongoPrefs);
    return hongoPrefs;
  }

  public static HongoPrefs getPreference() {
  ...
  public static DatabaseSpec registerSpec(String specId, int version) {
    ...
    if (DB_SPECS.containsKey(specId)) {
      throw new IllegalArgumentException(
          "There is a specification already created with " + specId + ". Use getDatabaseSpec to retrieve an existent specification.");
    }
   ...
   * @return the database specification registered with that id if any,  throws an {@link
  ...
  public static DatabaseSpec getDatabaseSpec(String specId) {
    ...
    throw new IllegalStateException(
        "There is no database specification for " + specId + ". Remeber: you must have already register it with registerDatabaseSpec method.");
  }
}
