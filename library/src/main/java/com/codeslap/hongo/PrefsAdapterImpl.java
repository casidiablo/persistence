...
class PrefsAdapterImpl implements PreferencesAdapter {
  ...
  @Override public <T> void store(T bean) {
    Class<?> theClass = bean.getClass();
    if (!HongoConfig.getPreference(mName).belongsToPreferences(theClass)) {
  ...
  @Override public <T> T retrieve(Class<T> theClass) {
    T bean;
    try {
  ...
  @Override public <T> boolean delete(Class<T> theClass) {
    SharedPreferences.Editor editor = getSharedPreferences(theClass).edit();
    for (Field field : theClass.getDeclaredFields()) {
