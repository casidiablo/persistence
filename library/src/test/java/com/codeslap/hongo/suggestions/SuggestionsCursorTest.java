...
public class SuggestionsCursorTest {
  ...
  public void suggestionsCursorTest() {
    Cursor cursor = new SuggestionsCursor(Arrays.asList(new SuggestionInfo.Builder().setId(1).setText("Foo").setDescription("Baz").build(), new SuggestionInfo.Builder().setId(2).setText("Bar").setDescription("Bal").build()));
  ...
  public void suggestionsCursorNoDescriptionTest() {
    Cursor cursor = new SuggestionsCursor(Arrays.asList(new SuggestionInfo.Builder().setId(1).setText("Foo").build(), new SuggestionInfo.Builder().setId(2).setText("Bar").build()));
    assertEquals(2, cursor.getCount());
    assertTrue(cursor.moveToFirst());
    assertEquals(1, cursor.getLong(SuggestionsCursor.COLUMN_INDEX_ID));
    assertEquals("Foo", cursor.getString(SuggestionsCursor.COLUMN_INDEX_TEXT));
