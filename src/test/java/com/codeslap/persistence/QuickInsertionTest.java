...
public class QuickInsertionTest extends SqliteTest {

    @Test
    public void testSingleInsertion() {
        assertNull(getQuickAdapter().store(null));
        ...
        Object id = getQuickAdapter().store(foo);
        ...
        List<ExampleAutoincrement> all = getQuickAdapter().findAll(ExampleAutoincrement.class);
    ...
    public void testCollectionInsertion() {
        ...
        getQuickAdapter().storeCollection(collection, null);

        // it should have stored all items
        assertEquals(collection.size(), getQuickAdapter().count(ExampleAutoincrement.class));

        // now let's see if it stored everything
        for (ExampleAutoincrement exampleAutoincrement : collection) {
            ExampleAutoincrement found = getQuickAdapter().findFirst(exampleAutoincrement);
            assertNotNull(found);
        ...
        collection = getQuickAdapter().findAll(ExampleAutoincrement.class);
        ...
        // it should have stored all items
        getQuickAdapter().storeUniqueCollection(collection, null);
        assertEquals(collection.size(), getQuickAdapter().count(ExampleAutoincrement.class));
